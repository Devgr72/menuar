...
import { ArrowLeft, BarChart, Box, Check, Images, IndianRupee, LayoutDashboard, QrCode, ShieldCheck, Eye, EyeOff } from "lucide-react";
...
export default function AuthPage({ mode }: Props) {
  const [activeMode, setActiveMode] = useState<"sign-in" | "sign-up" | "forgot-password">(mode as any);
  ...
  useEffect(() => {
    ...
    else if (location.pathname.includes("sign-up")) setActiveMode("sign-up");
  }, [location.search, location.pathname]);

  const toggleMode = (m: "sign-in" | "sign-up" | "forgot-password") => {
    setActiveMode(m);
    setFormError(null);
    setFormSuccess(null);
    ...
    if (m === "sign-in") navigate("/sign-in");
    else if (m === "sign-up") navigate("/sign-up");
  };
  ...
  async function handleForgotPasswordSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) {
        throw new Error(error.message ?? "Could not send reset link");
      }
      setFormSuccess("Reset link sent! Please check your email.");
    } catch (err) {
  ...
  const isForgot = activeMode === "forgot-password";
        ...
        <div className="grid w-full max-w-[1000px] overflow-hidden rounded-card border border-dd-line bg-white shadow-card lg:grid-cols-[minmax(0,42%)_minmax(0,58%)]">
          ...
          <div className="flex flex-col justify-center p-6 sm:p-8 lg:px-10 lg:py-8 relative">
            ...
            <p className="mt-1.5 text-[13.5px] text-dd-muted">
              {isForgot ? "Enter your email to receive a password reset link." : isSignUp ? "Start your AR menu in a few minutes." : "Sign in to your restaurant dashboard."}
            </p>
            ...
            {needsVerification ? (
              ...
              <>
                ...
                <form onSubmit={isForgot ? handleForgotPasswordSubmit : handleEmailSubmit} className="mt-5 flex flex-col gap-3">
                  {isSignUp && (
                    ...
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center justify-between">
                        ...
                        {!isSignUp && (
                          <button
                            ...
                            className="text-[11px] font-semibold text-dd-orange hover:underline"
                          >
                            Forgot password?
                          </button>
                        )}
                      ...
                      <div className="relative">
                        ...
                        <button
                          ...
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-dd-muted hover:text-dd-navy focus:outline-none"
                        >
                ...
                <p className="mt-5 text-center text-[13px] text-dd-muted">
                  ...
                  <button
                    type="button"
                    onClick={() => toggleMode(isForgot || isSignUp ? 'sign-in' : 'sign-up')}
                    className="font-semibold text-dd-orange hover:underline"
                  >
                    {isForgot || isSignUp ? "Please sign in" : "Sign up"}
                  </button>
                </p>
              </>
            )}
